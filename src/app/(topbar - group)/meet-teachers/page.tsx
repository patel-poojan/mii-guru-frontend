...
export default function Index() {
  ...
  const [openedTeacherId, setOpenedTeacherId] = useState<number | null>(1);
  console.log("openedTeacher", openedTeacherId);
  return (
    <>
          ...
          {faculty.map((teacher) => (
            <Dialog key={teacher.id}>
              ...
              <DialogContent className="h-[96%] w-[96%] rounded-xl md:rounded-2xl p-3 md:p-8 overflow-y-auto">
                <DialogHeader>
                  <DialogTitle className="text-xl text-left font-[600] tracking-normal  border-b-2 pb-4  border-[var(--MainLight-color)]">
                    Study Plan & Syllabus{" "}
                    <span className="text-[var(--MainLight-color)] ml-2">
                      {" "}
                      {
                        faculty.find(
